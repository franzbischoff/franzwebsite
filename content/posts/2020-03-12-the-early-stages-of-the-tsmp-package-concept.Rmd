...
Matrix Profile it's like a DM but faster (much faster) to compute. Figure \@ref(fig:matrix) shows a DM and a Matrix Profile. As you can see, in the Matrix Profile, as the name suggests, you see the Profile of a DM. It stores the minimum Euclidean distance of every subset of one TS (think of a Sliding Window) with another (or itself, called Self-Join). It also stores a companion vector called Profile Index, that gives us the index of each nearest neighbor.

```{r matrix, echo=FALSE, fig.cap="Distance Matrix and its Matrix Profile.", out.width=350, out.height=350, message=FALSE, warning=FALSE, fig.align = 'center'}
knitr::include_graphics("/posts/2020-03-12-the-early-stages-of-the-tsmp-package-concept_files/mp_1.png", error = FALSE)
```
...
Once become more familiar with this concept, you can start reading the [TSMP R Package](https://matrix-profile-foundation.github.io/tsmp/) documentation.
